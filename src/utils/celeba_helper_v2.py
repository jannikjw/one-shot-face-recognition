...
class CelebADatasetTriplet(CelebADataset):
    def __init__(self, root_dir, mapping_file: str, transform=None, 
        ...
        #test_image_names = self.file_label_mapping[self.file_label_mapping["is_train"]==0]["file_name"].reset_index(drop=True)

        # train, test dataframe
        self.train_df = self.file_label_mapping[self.file_label_mapping["is_train"]==1].reset_index(drop=True)
        test_df = self.file_label_mapping[self.file_label_mapping["is_train"]==0].reset_index(drop=True)
        ...
        real_test = pd.DataFrame.copy(test_df)
        real_test['count'] = real_test.groupby("person_id")['person_id'].transform('size')
        real_test = real_test[real_test['count'] >=3]
        real_test = real_test.groupby('person_id', sort=False).sample(n=3, random_state=42).sort_values(by='file_name')
        
        # new test_df
        self.test_df = test_df.drop(real_test.index, axis=0, inplace=False).reset_index(drop=True)
        self.test_image_names = self.test_df['file_name']
        
        #real_test set
        #self.real_test_image_names = test_image_names[real_test.index].reset_index(drop=True)
        self.real_test_df = real_test.iloc[:,:-1].reset_index(drop=True) # drop count column
        self.real_test_image_names = self.real_test_df['file_name']
    ...
    def __len__(self):
        if self.is_train:
            ...
            return int(0.05 * len(self.test_df))
        else:
            #return len(self.test_df)
            return len(self.real_test_df)


    def get_image_label(self, idx, get_train=True, real_test=False):
        ...
        if get_train:
            ...
            if idx >= len(self.real_test_image_names):
                print(idx)
            assert idx < len(self.real_test_image_names), "Index is out of range for real_test dataset"
            filename = self.real_test_image_names.loc[idx]
    ...
    def __getitem__(self, idx):
        if self.is_train or self.is_val:
            ...
            if self.is_train:
                anchor, anchor_label, anchor_name = self.get_image_label(idx, get_train=True, real_test=False)
            elif self.is_val:
                anchor, anchor_label, anchor_name = self.get_image_label(idx, get_train=False, real_test=False)
            ...
            if len(pos_list) == 0:
                ...
                positive, pos_label, pos_name = self.get_image_label(pos_idx, get_train=False, real_test=False)
            ...
            negative, neg_label, neg_name = self.get_image_label(neg_idx, get_train=False, real_test=False)

            return anchor, positive, negative, anchor_label

        else:
            anchor, anchor_label, anchor_name = self.get_image_label(idx, get_train=False, real_test=True)
